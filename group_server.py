...
def handle_client(conn, addr):
    ...
    try:
        while True:
            ...
            elif action == 'message':  # One-to-one message
            ...
            elif action == 'group_message':  # NEW: Group message action
                token = message.get('session')
                sender_username = validate_session(token)
                group_id = message.get('group_id')
                encrypted_content_b64 = message.get('encrypted_content')
                # ephemeral AES key encrypted with group_secret_key
                encrypted_message_aes_key_b64 = message.get(
                    'encrypted_message_aes_key')

                if sender_username and sender_username == current_username and group_id and encrypted_content_b64 and encrypted_message_aes_key_b64:
                    ...
                    for member in members:
                        if member in online_users:  # Only send to online members
                            send_json(online_users[member], group_message_data)
                    # send_json(conn, {'status': 'ok', 'message': 'Group message sent.'}) # Optional: sender confirmation
                else:
                    send_json(conn, {
                              'status': 'fail', 'message': 'Missing group message details or unauthorized.'})
